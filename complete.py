...
def follower_constraint_value_indiv(act_user, t, load_matrix, operator_action=None, user_action=None):
    ...
    for i in range(t):
        ...
        # ESS SOH Constraints
            if i == 0:
                x[7 * j][i] = - q_min/act_user - beta_s * x_s[j, i] + beta_b * x_b[j, i]
                x[7 * j + 1][i] = q_min/act_user + beta_s * x_s[j, i] - beta_b * x_b[j, i] - q_max/act_user
            else:
                x[7 * j][i] = - alpha * (-x[7 * j, i-1]) - beta_s * x_s[j, i] + beta_b * x_b[j, i]
                x[7 * j + 1][i] = alpha * (-x[7 * j, i-1]) + beta_s * x_s[j, i] - beta_b * x_b[j, i] - q_max/act_user
...
def follower_action_indiv(act_user, t, a_o, load, indiv=False):

    if total_user == act_user:
        load_active = load
        load_passive = np.zeros((1, t))
    else:
        load_active = load[:act_user, :]
        load_passive = load[act_user:, :]
    ...
    ess_matrix = np.fromfunction(np.vectorize(lambda i, j: 0 if i < j else np.power(alpha, i - j)),
                                 (t, t), dtype=float)
    q_ess = q_min/act_user * np.fromfunction(np.vectorize(lambda i, j: np.power(alpha, i + 1)), (t, act_user), dtype=float)\
...
def direction_finding_indiv(act_user, t, load_matrix, operator_action, user_action):
    ...
    follower_gradient_matrix = follower_constraints_derivative_matrix_indiv(act_user, t)
    follower_constraints_value = follower_constraint_value_indiv(act_user, t, load_matrix, operator_action, user_action)
    d = cp.Variable(1)
...
def step_size_indiv(act_user, t, load_matrix, operator_action, user_action, d, r):
    update_coef = 0.9
    s = 100
    for i in range(10000):
        #print(i)
        next_operator_action = operator_action + s * r
        result, x_s, x_b, l, taken_time = follower_action_indiv(act_user, t, next_operator_action, load_matrix)
...
def iterations_indiv(act_user, t, load, operator_action):
    a_o = operator_action
    result, x_s, x_b, l, taken_time = follower_action_indiv(act_user, t, a_o, load)
    ...
    for i in range(max_iter):
        print(i)
        print("dir")
        result, d, r, v, g = direction_finding_indiv(act_user, t, load, a_o, a_f)
        print("d :", result)
        print("step")
        b, _, _ = step_size_indiv(act_user, t, load, a_o, a_f, d, r)
        a_o = a_o + b * r
        print("step size : ", b)
        print("follower")
        result, x_s, x_b, l, taken_time = follower_action_indiv(act_user, t, a_o, load)
